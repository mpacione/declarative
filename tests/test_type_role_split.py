...
class TestSlotFlatteningForInlineRender:
    """Root cause fix for the 13-screen iPad cluster drift.

    ``build_semantic_tree`` moves ``element["children"]`` into
    ``element["slots"]`` for classified nodes with slot schemas. The
    compressor (``_build_node``) historically read children only,
    silently dropping slotted kids when the node renders inline
    (non-comp-ref). This test locks in the slot-flattening behaviour.

    See feedback_ipad_component_frame_inlining.md and the session
    handoff 2026-04-22 for the full diagnosis.
    """

    def test_inline_frame_with_slots_emits_slotted_children_in_markup(self) -> None:
        from dd.compress_l3 import compress_to_l3_with_maps
        from dd.markup_l3 import emit_l3
        spec = {
            "version": "1.0",
            ...
            "elements": {
                ...
                "btn-1": {
                    "type": "frame",
                    "role": "button",
                    "_original_name": "button-x",
                    "slots": {
                        "leading_icon": ["icon-a"],
                        "label": ["text-a"],
                        "trailing_icon": ["icon-b"],
                    },
                },
                "icon-a": {
                    "type": "instance",
        ...
        doc, *_ = compress_to_l3_with_maps(spec)
        markup = emit_l3(doc)
        # The btn's slot children must appear in markup — not dropped
        assert "icon-leading" in markup or "#icon-a" in markup, (
            f"Missing leading_icon slot child in markup. Got:\n{markup}"
        )
        assert "label" in markup.lower() and "click me" in markup.lower(), (
            f"Missing label slot text in markup. Got:\n{markup}"
        )
        assert "icon-trailing" in markup or "#icon-b" in markup, (
            f"Missing trailing_icon slot child in markup. Got:\n{markup}"
        )

    def test_slotted_children_preserved_in_order(self) -> None:
        """Slot-definition order (dict insertion order) is preserved
        ...
        spec = {
            ...
            "elements": {
                ...
                "c1": {"type": "text", "props": {"text": "LEAD"}, "_original_name": "lead"},
                "c2": {"type": "text", "props": {"text": "LABEL"}, "_original_name": "mid"},
                "c3": {"type": "text", "props": {"text": "TRAIL"}, "_original_name": "trail"},
            },
            "tokens": {},
            "_node_id_map": {},
        }
        doc, *_ = compress_to_l3_with_maps(spec)
        markup = emit_l3(doc)
        # Order check: LEAD before LABEL before TRAIL
        pos_lead = markup.find("LEAD")
        pos_label = markup.find("LABEL")
        pos_trail = markup.find("TRAIL")
        assert -1 < pos_lead < pos_label < pos_trail, (
            f"Slot order not preserved. lead={pos_lead} label={pos_label} "
            f"trail={pos_trail}\n{markup}"
        )
