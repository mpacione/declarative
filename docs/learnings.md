...
### Table-Driven Emission Is the Architectural Fix — IMPLEMENTED (Session 3)
Inspired by LLVM TableGen: define each property's emit behavior in the registry table, not in handcrafted emit functions. Simple properties use string templates: `'{var}.strokeAlign = "{value}";'`. Complex properties (fills, fontName, cornerRadius) use `emit={"figma": None}` and are handled by custom functions. `emit_from_registry(var, visual, renderer="figma")` iterates all registry entries, applies matching templates, returns JS lines. `_emit_visual` delegates to it. Structural test `tests/test_property_registry.py::TestRegistryEmitCoverage` enforces coverage — adding a property without an emit pattern fails the test. All 17 gaps closed, 34 new tests, 1,607 total passing.
