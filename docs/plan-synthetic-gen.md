...
| **M7.1** ✅ | **Edit grammar — all seven verbs at once.** Parser productions in `dd/markup_l3.py`; `EditStatement` AST types; `apply_edits(doc, [stmt]) → doc'` engine; unit tests per verb. No LLM. | All 7 verbs parse + apply correctly on minimal fixtures; each verb × each common case has a passing unit test. **Shipped 2026-04-19/20 in 9 passes (commits `39aa39e` → `fb693e1`); 109 passing + 1 skipped tests. See `docs/archive/plan-m7.1.md` + `docs/m7_assumptions_log.md`.** |
...
| **M7.0.d** ✅ | Forces/context per-instance LLM label. | Claude labels each instance's compositional role (e.g., "main-cta in login-form"). Alexander's overfitting guard. Adds a column to `screen_component_instances`. **Shipped 2026-04-21 late (commit `dabe52f`). Migration 019 adds `compositional_role TEXT`. `dd/forces.py` targets 16 load-bearing canonical_types (button / icon_button / card / header / heading / image / list_item / tabs / field_input / drawer / slider / select / toolbar / nav / chip / menu_item), trust-filters via the same consensus-method set the swap demo uses, batches 10 rows per Claude Haiku tool-use call with enum-pinned sci_id. Sanitises roles+contexts to kebab-case. Pilot on 20 Dank buttons produced clean labels: `main-cta in login-form`, `tertiary-action in toolbar`, `destructive-action in toolbar`, `confirm-action in toolbar`, `action-group in modal-header`. 10 tests.** |
| **M7.0.e** ✅ | Cross-screen pattern extraction. | Rule-of-three detection of repeating subtrees. Populates `patterns`. **Shipped 2026-04-21 late (commit `ea3a151`). `dd/patterns.py` collects `(parent_canonical_type, tuple(child_canonical_types))` one-level shape signatures from load-bearing structural parents (toolbar / nav / header / card / list_item / etc.); groups by distinct-screen count; filters by `min_screens=3`; asks Claude Haiku to label each with a regex-constrained slash-delimited name + 9-enum category + 20–240-char description; persists via `UNIQUE(name)` idempotent INSERT. Pilot on Dank: 13 candidates, all 13 labeled + persisted. Sample: `card/four-column-containers`, `card/container-with-dual-cards-and-action`, `drawer/text-content`, `tabs/four-buttons`, `header/image-select-toolbar`. 10 tests.** |
| **M7.0.f** ✅ | Sticker-sheet-authoritative tagging (where present, e.g., Frame 429/430 in Dank). | Reconciles with M7.0.b/c output; when present, sticker sheet wins. Skipped silently on projects without a sticker sheet. **Shipped 2026-04-21 late (commit `bb1175b`). Migration 020 adds `authoritative_source TEXT` to `components`. `dd/sticker_sheet.py` detects sticker-sheet screens by `screen_type='design_canvas'` (configurable), collects distinct INSTANCE component_keys on them, resolves through CKR when `components` is keyed on `figma_node_id` (Dank production schema) vs direct `component_key` column (test fixtures), and UPDATEs `authoritative_source='sticker_sheet'`. Idempotent; splits `unknown_component_keys` (CKR miss) from `unregistered_components` (CKR hit, components miss) for diagnostics. Pilot on Dank: 3 sticker-sheet screens (Frames 429/430 + one more), 121 candidate keys, 94 tagged (80 icons, 3 buttons, 1 tabs, 1 image, 1 header, 8 unclassified). 7 tests. Reconciliation of sticker-sheet slot/variant data vs M7.0.b/c heuristic output is deferred to a follow-up.** |
