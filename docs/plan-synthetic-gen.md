...
| **M7.2** 🟢 | **First LLM-in-loop demo — S2.5 component swap.** Library-intensive: exercises slot definitions + variant families + the `swap` verb end-to-end via Claude tool-use. | Claude receives a screen's L3 + library catalog as tool context. Emits `swap @X with=-> family/variant` as a tool call. Apply + render + verify: `is_parity=True` + the resolved component matches the requested variant. **Infrastructure complete (2026-04-21 late): structural verify landed (commits `fbda4d3` → `90d9a94`) and render+bridge+verify wiring landed (commit `61be69b`, see `dd/apply_render.py`: `rebuild_maps_after_edits` carries compressor side-car maps across `apply_edits` splices; `adjust_spec_elements_for_edits` null-clears visuals so the verifier skips fills/strokes/effects comparison where the new master supplies its own; `render_applied_doc` wraps the three-step pipeline; `walk_rendered_via_bridge` subprocess-wraps `render_test/walk_ref.js`). `scripts/m7_swap_demo.py` gains `--render` / `--skip-bridge` / `--bridge-timeout` / `--save-script` flags; exit codes 0 = is_parity pass, 1 = structural/verify fail, 2 = render-OK but bridge unavailable. Dry-run on Dank screen 183 generates a 52kB script structurally identical to the 204/204 baseline except for the swapped CompRef. **Live-bridge gate deferred**: walk currently hits the 170s PROXY_EXECUTE timeout on a loaded plugin state (known env issue per `feedback_sweep_transient_timeouts.md`). Retry through a fresh bridge state (restart Figma plugin) to finish the live is_parity=True capture.** |
