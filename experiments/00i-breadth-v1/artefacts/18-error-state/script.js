...
n3.name = "button-1";
{ const _t = n3.findOne(n => n.type === "TEXT" && /^(title|label|heading)$/i.test(n.name)) || n3.findOne(n => n.type === "TEXT"); if (_t) { await figma.loadFontAsync(_t.fontName); _t.characters = "Retry"; } }
M["button-1"] = n3.id;
...
n2.appendChild(n6);
try { n6.characters = "Something went wrong"; } catch (__e) { __errors.push({eid:"heading-1", kind:"text_set_failed", error: String(__e && __e.message || __e)}); }
n6.layoutSizingHorizontal = "FILL";
n2.appendChild(n7);
try { n7.characters = "We apologize for the inconvenience. Please try again or contact support if the problem persists."; } catch (__e) { __errors.push({eid:"text-1", kind:"text_set_failed", error: String(__e && __e.message || __e)}); }
