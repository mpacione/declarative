...
n17.name = "button-2";
{ const _t = n17.findOne(n => n.type === "TEXT" && /^(title|label|heading)$/i.test(n.name)) || n17.findOne(n => n.type === "TEXT"); if (_t) { await figma.loadFontAsync(_t.fontName); _t.characters = "Maybe"; } }
M["button-2"] = n17.id;

const n18 = await (async () => { const __src = _p2; if (!__src) { __errors.push({eid:"button-3", kind:"missing_component_node", id:"5749:82457"}); return _missingComponentPlaceholder("button-3", 24, 24, "button-3"); } try { return __src.createInstance(); } catch (__e) { __errors.push({eid:"button-3", kind:"create_instance_failed", id:"5749:82457", error: String(__e && __e.message || __e)}); return _missingComponentPlaceholder("button-3", 24, 24, "button-3"); } })();
n18.name = "button-3";
{ const _t = n18.findOne(n => n.type === "TEXT" && /^(title|label|heading)$/i.test(n.name)) || n18.findOne(n => n.type === "TEXT"); if (_t) { await figma.loadFontAsync(_t.fontName); _t.characters = "Can't Go"; } }
M["button-3"] = n18.id;
...
n28.primaryAxisAlignItems = "CENTER";
...
n32.primaryAxisAlignItems = "CENTER";
n32.counterAxisAlignItems = "CENTER";
n32.fills = [{type: "SOLID", color: {r:0.8863,g:0.9098,b:0.9412}}];
n32.cornerRadius = 999;
...
n1.appendChild(n6);
n1.appendChild(n7);
try { n7.characters = "Event Details"; } catch (__e) { __errors.push({eid:"text-1", kind:"text_set_failed", error: String(__e && __e.message || __e)}); }
n7.layoutSizingHorizontal = "FILL";
n1.appendChild(n8);
n2.appendChild(n9);
try { n9.characters = "Summer Music Festival 2024"; } catch (__e) { __errors.push({eid:"heading-1", kind:"text_set_failed", error: String(__e && __e.message || __e)}); }
...
n19.appendChild(n24);
n19.appendChild(n25);
try { n25.characters = "Saturday, July 20, 2024 • 6:00 PM"; } catch (__e) { __errors.push({eid:"text-2", kind:"text_set_failed", error: String(__e && __e.message || __e)}); }
n25.layoutSizingHorizontal = "FILL";
n20.appendChild(n26);
n20.appendChild(n27);
try { n27.characters = "Central Park, New York, NY"; } catch (__e) { __errors.push({eid:"text-3", kind:"text_set_failed", error: String(__e && __e.message || __e)}); }
